    ...
    try {
        ...
        case 'stiker':
            if (isMedia) {
                ...
                await client.reply(from, 'No picture! To open a list of send orders #menu', id)
            }
            break
        case 'stikergif':
        case 'stickergif':
        case 'gifstiker':
        case 'gifsticker':
            if (args.length !== 1) return client.reply(from, 'Sorry, the message format is wrong, please check the menu.', id)
            if (isGiphy) {
        ...
        case 'facebook':
            ...
            facebook(url)
                ...
                .catch((err) => {
                    client.reply(from, `Error, Invalid url or not loading video \n\n${err}`, id)
                })
            break
        // Other Command
        case 'mim':
        ...
        case 'ocr':
            if (isMedia) {
                ...
                if (!url.match(isUrl)) await client.reply(from, 'Sorry, the message format is wrong, please check the menu.', id)
        ...
        case 'kick':
            ...
            if (!isBotGroupAdmins) return client.reply(from, 'Failed, please add the bot as a group admin!', id)
            if (mentionedJidList.length === 0) return client.reply(from, 'Sorry, the message format is wrong, please check the menu.', id)
            await client.sendText(from, `Request received, issued:\n${mentionedJidList.join('\n')}`)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if (groupAdmins.includes(mentionedJidList[i])) return await client.sendText('Failed, you cannot remove the group admin.')
                await client.removeParticipant(groupId, mentionedJidList[i])
            }
            break
        case 'promote': {
            if (!isGroupMsg) return await client.reply(from, 'Sorry, this command can only be used within groups!', id)
            if (!isGroupAdmins) return await client.reply(from, 'Failed, this command can only be used by group admins!', id)
            if (!isBotGroupAdmins) return await client.reply(from, 'Failed, please add the bot as a group admin!', id)
            if (mentionedJidList.length === 0) return await client.reply(from, 'Sorry, the message format is wrong, please check the menu.', id)
            if (mentionedJidList.length >= 2) return await client.reply(from, 'Sorry, this command can only be applied to 1 user.', id)
        ...
        case 'demote': {
            if (!isGroupMsg) return client.reply(from, 'Sorry, this command can only be used within the group!', id)
            if (!isGroupAdmins) return client.reply(from, 'Failed, this command can only be used by group admins!', id)
            if (!isBotGroupAdmins) return client.reply(from, 'Failed, please add the bot as a group admin!', id)
            if (mentionedJidList.length === 0) return client.reply(from, 'Sorry, the message format is wrong, please check the menu.', id)
            if (mentionedJidList.length >= 2) return await client.reply(from, 'Sorry, this command can only be applied to 1 user.', id)
            if (!groupAdmins.includes(mentionedJidList[0])) return await client.reply(from, 'Sorry, the user is not an admin.', id)
            await client.demoteParticipant(groupId, mentionedJidList[0])
            await client.sendTextWithMentions(from, `Request accepted, remove position @${mentionedJidList[0].replace('@c.us', '')}.`)
            break
        }

        case 'bye':
            if (!isGroupMsg) return client.reply(from, 'Sorry, this command can only be used within groups!', id)
            if (!isGroupAdmins) return client.reply(from, 'Sorry, this command can only be performed by group admins!', id)
